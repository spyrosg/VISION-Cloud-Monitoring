...
public class FieldCheck {
     ...
     * @param isInner
     */
    FieldCheck(final String field, final boolean exact, final String value, final boolean isInner) {
        if( isInner )
     ...
     * @param inner
     */
    FieldCheck(final String field, final FieldCheck inner) {
        this.field = EventField.valueOf( field );

        if( !this.field.hasInner() )
            throw new AssertionError( "Field '" + field + "' specified does not have inner fields." );
     ...
     */
    public boolean sameFieldWith(final FieldCheck chk) {
        if( field != chk.field )
            return false;
        if( inner == null ^ chk.inner == null )
            return false;
        if( inner != null ) //
            if( inner.field != chk.inner.field )
                return false;
        return true;
    }
